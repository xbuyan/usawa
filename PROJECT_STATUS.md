...
Verified against real infrastructure (a real Redis server, real migration
against a table with pre-existing rows), not just written and assumed to
work — per this project's own honesty standard.

1. **Redis-backed rate limiting.** This closes a gap that was live in
   production, not hypothetical: `render.yaml`/`Procfile` already ran 2
   gunicorn workers while `extensions.py` used `storage_uri="memory://"`,
   meaning rate limits were being enforced per-worker, not globally — the
   real effective limit and which requests got through was inconsistent.
   Fixed via `REDIS_URL` (falls back to in-memory with a loud startup
   warning if unset — safe for local dev, wrong for production).
   `render.yaml` now provisions a Render key-value (Redis) instance.
   **Tested against a real Redis server**, with a test that reproduces the
   original bug (two independent processes on `memory://` do NOT share a
   counter) alongside proof of the fix (two independent processes on
   Redis DO share a counter).
2. **Account lockout.** 5 consecutive failed logins locks the account for
   15 minutes, independent of source IP (a second layer alongside rate
   limiting, which only slows brute force by IP). Real Alembic migration
   generated and — this matters — **tested against a table with a
   pre-existing row**, which caught a real bug: the naive autogenerated
   migration used `nullable=False` with no default, which fails against
   any non-empty table on both SQLite and Postgres. Fixed with
   `server_default='0'`. Also caught and fixed a naive/aware datetime
   comparison bug (SQLite strips timezone info on read) that a shallower
   test wouldn't have surfaced.
3. **Flexible CSV column mapping.** A synonym-based header mapper (not
   fuzzy/similarity matching — deliberately, since silently mis-mapping a
   salary or demographic column would produce a confidently wrong equity
   score, which is worse than a loud error). Exact template column names
   still work unchanged; common real-world spellings ("Employee ID",
   "Job Level", "Base Salary", etc.) are now recognized; a genuinely
   unrecognized required column now fails with a clear, specific error
   instead of silently skipping every row.
4. **Dependency vulnerability scanning.** Running `pip-audit` against the
   actual pinned `requirements.txt` found **3 real, known CVEs** in
   versions this project was running: Flask 3.0.3, python-dotenv 1.0.1,
   pytest 8.3.3. Bumped to patched versions (3.1.3 / 1.2.2 / 9.0.3), reran
   the full test suite against the patches (55/55 passed at the time, no
   regressions), and reran `pip-audit` to confirm clean. Added
   `.github/workflows/ci.yml` to run the test suite (against a real Redis
   service container) and `pip-audit` on every push/PR, so this doesn't
   silently drift again. **Honest caveat: this CI workflow has been
   verified command-by-command locally, but has not yet been observed
   actually running inside GitHub Actions** — push and check the Actions
   tab before fully trusting it (see README's "what's still missing").
5. **Audit log.** A durable, queryable `AuditLog` table (`models.py`,
   `audit_log.py`) — deliberately not just a log line, which the app
   already had for saves/deletes but wasn't queryable per-user, wasn't
   durable beyond whatever log retention the host gives for free, and
   didn't cover reads at all. Now records: login success / failure /
   lockout, employee and applicant CSV uploads, AI insights generation,
   and client-report create / view / delete. Scoped per-user like
   everything else; exposed via `GET /api/audit-log`. New Alembic
   migration generated and tested against a table with pre-existing
   rows (this one was a clean new-table migration, so it didn't hit the
   same NOT-NULL-default bug the lockout migration did — checked anyway
   rather than assumed).
   **Related bug found and fixed while building this:** the app had no
   `ProxyFix` middleware. Render terminates TLS at a reverse proxy, so
   without it, `request.remote_addr` — used by both the audit log's IP
   field and Flask-Limiter's rate-limiting key — would have returned the
   *proxy's* address for every request, not the real client's. In
   practice this would have meant every user sharing one effective
   IP-based rate-limit bucket. Added `ProxyFix(x_for=1)`, trusting
   exactly one hop (correct for Render's single-reverse-proxy setup).
   **Tested with a real `X-Forwarded-For` header**, confirming the
...
locally (`apt install redis-server` / `brew install redis`) and start it
before running the full suite, or those 2 tests will fail loudly rather
than silently skip (deliberate — a silent skip could let the original
rate-limiting bug slip back in unnoticed).
